...
// Tue Sep 26 15:24:38 2017 by ROOT version 6.06/08
// from TTree treeMaker/tree
// found on file: ../ntuple_EleData/NCUGlobalTuples_1.root
//////////////////////////////////////////////////////////

#ifndef my_MakeClass_h
#define my_MakeClass_h

#include <TROOT.h>
#include <TChain.h>
#include <TFile.h>

#include <iostream> // Yu-Hsiang add
#include <fstream>  // Yu-Hsiang add 
...
using namespace std; // Yu-Hsiang add
...
//   my_MakeClass(TTree *tree=0); // Yu-Hsiang comment
   my_MakeClass(std::string inputFile,TTree *tree=0); // Yu-Hsiang add
   virtual ~my_MakeClass();
   ...
   string inputFile_; //  Yu-Hsiang add

};

#endif

#ifdef my_MakeClass_cxx
//my_MakeClass::my_MakeClass(TTree *tree) : fChain(0) // Yu-Hsiang comment
my_MakeClass::my_MakeClass(std::string inputFile, TTree *tree) : fChain(0) // Yu-Hsiang add
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.

   inputFile_= inputFile; // Yu-Hsiang add

   if (tree == 0) {
//      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../ntuple_EleData/NCUGlobalTuples_1.root"); // Yu-Hsiang comment
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(inputFile.data()); // Yu-Hsiang add
      if (!f || !f->IsOpen()) {
//         f = new TFile("../ntuple_EleData/NCUGlobalTuples_1.root"); // Yu-Hsiang comment
        f = new TFile(inputFile.data()); // Yu-Hsiang add
      }
//      TDirectory * dir = (TDirectory*)f->Get("../ntuple_EleData/NCUGlobalTuples_1.root:/tree"); // Yu-Hsiang comment
      TDirectory * dir = (TDirectory*)f->Get(Form("%s:/tree",inputFile.data())); // Yu-Hsiang add
...
my_MakeClass::~my_MakeClass()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t my_MakeClass::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t my_MakeClass::LoadTree(Long64_t entry)
{
...
void my_MakeClass::Init(TTree *tree)
{
...
void my_MakeClass::Show(Long64_t entry)
{
...
// If entry is not specified, print current entry
  ...
  // Yu-Hsiang comment
Int_t my_MakeClass::Cut(Long64_t entry)
{
