...
TString dir_get_file = "text_and_ROOT_files" ;
  ...
  for(int i=0;i<N_cut_points;i++){
	...
	TString ROOT_file_path = Form("_cut_%d.root",cut_point[i]);
...
// setting
  ...
  TLegend *leg=new TLegend(0.75,0.20,1.0,0.85);
...
//        h_histo_for_cut_index[i]->GetXaxis()->SetRangeUser(500, 1700);
        h_histo_for_cut_index[i]->GetXaxis()->SetRangeUser(500, 4500);
...
//  if(variable_name == "Ele_1st_pt") save_dir = "optimize_cut_with_limit_use_eleIsPassVeto/leading_pt/";
  if(variable_name == "Ele_2nd_pt") save_dir = "optimize_cut_with_limit_use_eleIsPassVeto/subleading_pt/";
