...
void xAna_muo_selection(TString inputFile, TCanvas *c1 , TCanvas *c2 , TCanvas *c3 ,TCanvas *c4 ,TCanvas *c5 , TCanvas *c6 ,
...
//  TreeReader data(inputFile.data());
  ...
  Long64_t nTotal=0;
  Long64_t nPass[20]={0};
  Long64_t nMuonPairCounter=0;
  Long64_t nMuonPairCounter2=0;
  Long64_t nMuonPairCounter3=0;
  ...
  for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
    ...
    //4. look for good electrons first (change to Muon)
...
//    TClonesArray* eleP4 = (TClonesArray*) data.GetPtrTObject("eleP4");
    ...
    //5. select good electrons (change to Muon)
    ...
    //6. select a good Z boson (change to Muon)
  ...
  for(unsigned int i = 0; i < goodMuons.size(); i++){
    ...
    for(unsigned int j = 0; j < i; j++){
      ...
      // decide leading and sub leading
      int leadingIndex = pt1>pt2? im : jm;
      int subleadingIndex = pt1 > pt2? jm : im;

      // eta
      float leading_eta =-99;
      float subleading_eta =-99;

      if(leadingIndex == im)// im is leading mu, jm is subleading mu		
	{
	leading_eta = fabs( thisMu->Eta() );
	subleading_eta = fabs( thatMu->Eta() );
	}
      else if(leadingIndex == jm)// jm is leading mu, im is subleading mu            
        {
        leading_eta = fabs( thatMu->Eta() );
        subleading_eta = fabs( thisMu->Eta() );
        }

	if( leading_eta > 2.1) continue;
        if( subleading_eta > 2.4) continue;

      // other opposite charge, mll, ptll, further id cut 
      ...
      // save the leading muon pair only
      if( !findMPair ){
	goodMuID.push_back(leadingIndex);
        goodMuID.push_back(subleadingIndex);
      }
    ...
    if(goodMuID.size()>0)
    {
        TLorentzVector* Ele1 = (TLorentzVector*)muP4->At(goodMuID[0]);
        TLorentzVector* Ele2 = (TLorentzVector*)muP4->At(goodMuID[1]);
...
if( !isHighPtMuon[Mu1] && isCustomTrackerMuon[Mu1] && !isHighPtMuon[Mu2] && isCustomTrackerMuon[Mu2]  ){ 
//    nMuonPairCounter ++;
}
	...
	{
        TLorentzVector* thisEle = (TLorentzVector*)muP4->At(goodMuID[0]);
        if(thisEle->DeltaR(*thisJet)<0.8)hasOverLap=true;

        TLorentzVector* thatEle = (TLorentzVector*)muP4->At(goodMuID[1]);
...

{
      ...
      int ie = goodMuID[0];
    TLorentzVector* thisEle = (TLorentzVector*)muP4->At(ie);
    ...
    // save delta Eta, delta Phi and delta R BTW reco-Z and reco-H
      {
        ...
        int ie_1st = goodMuID[0];
        int ie_2nd = goodMuID[1];
    	TLorentzVector* thisEle = (TLorentzVector*)muP4->At(ie_1st);
    	TLorentzVector* thatEle = (TLorentzVector*)muP4->At(ie_2nd);
...
//      std::cout << "nMuonPairCounter:  " << nMuonPairCounter << std::endl;
//      std::cout << "nMuonPairCounter2:  " << nMuonPairCounter2 << std::endl;
//      std::cout << "nMuonPairCounter3:  " << nMuonPairCounter3 << std::endl;
