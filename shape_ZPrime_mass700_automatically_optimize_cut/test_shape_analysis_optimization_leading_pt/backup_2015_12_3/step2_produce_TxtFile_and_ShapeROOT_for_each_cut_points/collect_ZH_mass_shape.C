...
// scale the shape to event number in text file

  double data_number = 27;
double DY_event_number = 26.8807;//39.4041;
double signal_event_number[N_signal_mass_points] =
{
 6.57196, 
 5.20635,
 3.02693,
...
*/
  ...
  for(int i=0;i<N_signal_mass_points;i++){ 
        ...
        h_signal[i]->Sumw2();
	double scale = signal_event_number[i] / (h_signal[i]->Integral() ) ;
 	h_signal[i]-> Scale( scale );
//	cout<<"total h_signal[i]:"<< h_signal[i]->Integral() << endl;
  }

  // background
   h_background_combine->Sumw2();
   double scale = DY_event_number / (h_background_combine->Integral() ) ;
   h_background_combine-> Scale( scale );
//   cout<<"total h_background_combine:"<< h_background_combine->Integral() << endl;

   // data
  TH1D *h_data = (TH1D*) h_background_combine ->Clone("data_obs");

   h_data ->Sumw2();
   scale =  data_number / ( h_data->Integral() ) ;
   h_data -> Scale( scale );
...
TString ROOT_name = "Zprime_shape.root";
TFile *myFile = new TFile(ROOT_name,"recreate");
TString siganl_name[N_signal_mass_points];

  for(int i=0;i<N_signal_mass_points;i++){

	siganl_name[i] = Form("SIG_M%d",Zprime_mass[i]  ); 

        h_signal[i]->SetName(siganl_name[i]);
	h_signal[i]->Write();

  }


TString background_name ="DYJETS";
h_background_combine ->SetName(background_name);
h_background_combine ->Write();
