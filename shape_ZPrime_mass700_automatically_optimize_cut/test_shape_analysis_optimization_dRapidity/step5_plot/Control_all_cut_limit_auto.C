...
//  leg_name[i] = Form("CL_{S} Expected Limit for cut pt>%d",cut_value[i] ) ;
  leg_name[i] = Form("|#Delta Rapidity_{ZH}| <%f",cut_value[i] ) ;
  }
  ...
  TLegend *leg = new TLegend(.65, .50, 1.0, .90);
  leg->SetFillColor(0);
...
// end loop
  ...
  hr->SetYTitle("95% #sigma_{Z'}xBR(Z'->ZH) [pb]");
    ...
    latex->DrawLatex(0.15, 0.91, "CL_{S} Expected Limit, cut |#Delta Rapidity_{ZH}|");
