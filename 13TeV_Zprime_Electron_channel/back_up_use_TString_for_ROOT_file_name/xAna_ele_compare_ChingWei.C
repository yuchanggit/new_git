...
using namespace std;
void xAna_ele_compare_ChingWei(/*std::string*/ TString  inputFile, /*TCanvas *c1 , TCanvas *c2 , TCanvas *c3 ,TCanvas *c4 ,TCanvas *c5 , */
...
//TreeReader data(inputFile.data());
  ...
  Long64_t nSD20to220=0;
  Long64_t nSDwindow=0;
...
//    double SD_low =91, SD_high =141;
//    double SD_low =95, SD_high =145;
    double SD_low =93, SD_high =143;
    ...
    for(unsigned int i=0; i< SD_jet_index.size(); i++){
      ...
      if(SD_Mass[i]<SD_low || SD_Mass[i]>SD_high )continue;
      nSDwindow ++;
...
cout<< "nSD20to220: "<<nSD20to220 <<endl;
cout<< "nSDwindow: "<<nSDwindow <<endl;

double NSD20to220= nSD20to220;
double NSDwindow = nSDwindow;

double SD_window_eff = NSDwindow/NSD20to220;
cout<< "SD_window_eff: "<< SD_window_eff <<endl;
