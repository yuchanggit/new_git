...
//        int lowest_cut = 10;
//        int lowest_cut = 35; // for sub leading electron pt 
        int lowest_cut = 115; // for leading electron pt 
//        double lowest_cut = 0; // for |dEta| and |d Rapidity| 
	...
	for(int i=0;i<Nbins;i++){
  	...
  	TLegend *leg=new TLegend(0.6,0.35,0.99,0.5);
...
//  TCanvas *c4 = new TCanvas("2nd_ele_pT_distribuion_background_zoom_in","2nd_ele_pT_distribuion_background_zoom_in",200,10,700,500);
  int  colorN = 1;
  int  styleN = 3001;
  TLegend *leg2 = new TLegend(0.4,0.5,0.9,0.9);
...
//continue;
        ...
        leg5[k] = new TLegend(0.50,0.75,0.9,0.9);// for input
...
//        h_signal_varies_mass->GetXaxis()->SetTitle("Sub-leading electron p_{T} [ GeV]");
        h_signal_varies_mass->GetXaxis()->SetTitle("Leading electron p_{T} [ GeV]");
...
//	h_signal_varies_mass->GetXaxis()->SetRangeUser(0, 4.0);
	...
	double X_up_range = 1600;
	double Y_up_range_signal = 0.12;
        double Y_up_range_background = 0.15;
...
//        h_background_varies_mass->GetXaxis()->SetTitle("Sub-leading electron p_{T} [GeV]");
        h_background_varies_mass->GetXaxis()->SetTitle("Leading electron p_{T} [ GeV]");
