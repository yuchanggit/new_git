...
//	scan_width = 100;
 	int multipier = max_non_zero_bin / scan_width;
  	int highest_cut =  multipier * scan_width;
...
//                        double lumi = 1;
		        ...
		        for(int i=0;i<Nbins; i++)
                        {
                                ...
                                efficiency( background_eff[i] , background_eff_err[i] , h_background_2nd_ele_pT_combine_scale1  , ( i * scan_width ) + lowest_cut + 1 );
			...
			h_background_2nd_ele_pT_combine_scale2->Scale( lumi );
                        ...
                        for(int i=0;i<Nbins; i++)
                        {
                                // this example applies when you require a minimum threshold of value x

                              	significance( sig[i] , sig_err[i] , h_2nd_ele_pT_ , h_background_2nd_ele_pT_combine_scale2 ,  ( i * scan_width ) + lowest_cut +1);

				vy_significance[i] = sig[i];
				vy_significance_erry[i] = sig_err[i];
//				cout<<"sig[i]:  "<< sig[i]  <<endl;
//                                cout<<"sig_err[i]:  "<<  sig_err[i] <<endl;
