...
from ROOT import TStyle, TCanvas, TPad, TLegend, TLatex, TText, TH1D
...
#        print "iGen_value:", iGen_value.getVal()
        ...
        h_Pull = TH1D("h_Pull","h_Pull",25,-5,5);

        Jmass_frame4 = J_mass.frame(RooFit.Title("test frame4"))

	times_max = 1000

	# modelVjet_test's parameters are fixed to generate pseudo-data
        constVjet_test.setConstant(True) 
        offsetVjet_test.setConstant(True)
        widthVjet_test.setConstant(True)

	# parameter used in fit later
	...
	constVjet_value =-0.020 	
	a0Vjet_value = -0.1 
	a1Vjet_value =  0.6
	a2Vjet_value = -0.1

        constVjet_test4   = RooRealVar("constVjet_test4",   "slope of the exp", constVjet_value , -1.,   0.)
    	a0Vjet_test4 = RooRealVar("a0Vjet_test4", "width of the erf", a0Vjet_value, -5, 0)
    	a1Vjet_test4 = RooRealVar("a1Vjet_test4", "width of the erf", a1Vjet_value,  0, 5)
    	a2Vjet_test4 = RooRealVar("a2Vjet_test4", "width of the erf", a2Vjet_value, -1, 1)
...
#		print "constVjet_value_fit_MC:",constVjet_value_fit_MC
		...
		# modelVjet_test3's parameter in fit reset in value_fit_MC
...
#        	print "Bias_value2 of VR+SR:", Bias_value2
		...
		# calculate the pull
                ...
                iGen_All = modelVjet_test.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("h_reasonable_range"))	
		nGen_SR = Entries_pseudo_data_fluc * (iGen_value2.getVal()/iGen_All.getVal()) 

                iFit_All = modelVjet_test3.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("h_reasonable_range"))
                nFit_SR = Entries_pseudo_data_fluc * (iFit_value2.getVal()/iFit_All.getVal())

#		print "Entries_pseudo_data_fluc:",Entries_pseudo_data_fluc
#		print "iGen_All:",iGen_All.getVal()," iGen_value2:",iGen_value2.getVal()," nGen_SR:",nGen_SR
#                print "iFit_All:",iFit_All.getVal()," iFit_value2:",iFit_value2.getVal()," nFit_SR:",nFit_SR

		sigma_Gen = math.sqrt( nGen_SR ) 
		pull_value = ( nFit_SR - nGen_SR )/ sigma_Gen
#		print "sigma_Gen:", sigma_Gen," pull_value:", pull_value

		h_Pull.Fill( pull_value )
		...
		# reset parameter
		constVjet_test4.setVal(constVjet_value )
		a0Vjet_test4.setVal(a0Vjet_value )
		a1Vjet_test4.setVal( a1Vjet_value)
		a2Vjet_test4.setVal(a2Vjet_value )
  	 
		modelVjet_test4 = RooExponential("modelVjet_test4", "exp for V+jets mass", J_mass, constVjet_test4)
#		modelVjet_test4 = RooChebychev("modelVjet_test4", "polynomial for V+jets mass", J_mass, RooArgList(a0Vjet_test4, a1Vjet_test4, a2Vjet_test4))
#		modelVjet_test4 = RooGenericPdf("modelVjet_test4", "powerlaw for X mass", "@0^@1", RooArgList(J_mass, a0Vjet_test4))
...
#        c_test2.SaveAs(Save_Dir+"/"+"use_shape_to_generate_pseudo_data_test.pdf")
        ...
        c_test4.cd()
	h_Bias.Draw()
        c_test4.SaveAs(Save_Dir+"/"+"h_Bias2.pdf")

        c_test5 = TCanvas("test5","test draw 5",800,600)
        c_test5.cd()
        h_Pull.Draw()
        c_test5.SaveAs(Save_Dir+"/"+"h_Pull.pdf")
