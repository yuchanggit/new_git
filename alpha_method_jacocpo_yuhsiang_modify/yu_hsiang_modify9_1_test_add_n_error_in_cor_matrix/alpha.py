...
#    print "iFit_SR_error.getVal(): ", iFit_SR_error.getVal()
#    print "iFit_SB_error.getVal(): ", iFit_SB_error.getVal()
#    print "N_fit_central: ", N_fit_central
...
#    sigma_Fit = math.sqrt( resultM(0,0)  )
    ...
    # Consider the N_SB_events's error so add par3 term then it's 3x3 matrix

    N_SB_events_error = math.sqrt( N_SB_events )

    N_SB_events_plus = N_SB_events + N_SB_events_error
    N_SB_events_minus = N_SB_events - N_SB_events_error

    par1.setVal(par1_value )  # let iFit_SR_error iFit_SB_error change to central value 
    par2.setVal(par2_value )

    N_fit_nSB_plus = N_SB_events_plus * (  iFit_SR_error.getVal() / iFit_SB_error.getVal() )
    N_fit_nSB_minus = N_SB_events_minus * (  iFit_SR_error.getVal() / iFit_SB_error.getVal() )

    nFit_SR_shift_par3_nSB = max( abs( N_fit_nSB_plus - N_fit_central ) , abs( N_fit_central - N_fit_nSB_minus) )

#    print ""
#    print "test N_SB shift"
#    print "N_fit_central: ",N_fit_central
#    print "iFit_SR_error.getVal(): ",iFit_SR_error.getVal()
#    print "iFit_SB_error.getVal(): ",iFit_SB_error.getVal()  
#    print "N_SB_events: ", N_SB_events
#    print "N_SB_events_error: ", N_SB_events_error
#    print "N_SB_events_plus: ", N_SB_events_plus
#    print "N_SB_events_minus: ",N_SB_events_minus
#    print "N_fit_nSB_plus: ", N_fit_nSB_plus
#    print "N_fit_nSB_minus: ",N_fit_nSB_minus
#    print "nFit_SR_shift_par3_nSB: ", nFit_SR_shift_par3_nSB
#    print ""

    # matrix manipulation (3x3 matrix)    

    rowM13 = TMatrixD(1,3)
    rowM13[0][0] = nFit_SR_shift_par1
    rowM13[0][1] = nFit_SR_shift_par2
    rowM13[0][2] = nFit_SR_shift_par3_nSB

    colM31 = TMatrixD(3,1)
    colM31[0][0] = nFit_SR_shift_par1
    colM31[1][0] = nFit_SR_shift_par2
    colM31[2][0] = nFit_SR_shift_par3_nSB

    corM33 = TMatrixD(3,3)

    corM33[0][0] = cor[0][0] # copy the 2x2 correlation matrix
    corM33[0][1] = cor[0][1]
    corM33[1][0] = cor[1][0]
    corM33[1][1] = cor[1][1]
