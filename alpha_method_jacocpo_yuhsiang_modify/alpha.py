...
#    entryHSB.setError(math.sqrt(entryHSB.getVal()))
    ...
    if channel == "XZhnnb":
    	...
    	constVjet_test   = RooRealVar("constVjet_test",   "slope of the exp",      -0.020, -1.,   0.)
    	offsetVjet_test  = RooRealVar("offsetVjet_test",  "offset of the erf",     30.,   -50., 200.)
    	widthVjet_test   = RooRealVar("widthVjet_test",   "width of the erf",     100.,     1., 200.)
...
#    	widthVjet_test.setConstant(True)
	...
	# make another dataset that remove the signal region 
        pseudo_data_SB = RooDataSet("pseudo_data_SB", "pseudo_data_SB", RooArgSet(J_mass), RooFit.Import(pseudo_data), RooFit.Cut("J_mass<65 || J_mass>135") )
