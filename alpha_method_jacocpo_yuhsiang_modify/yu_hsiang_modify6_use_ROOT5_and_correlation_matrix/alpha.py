...
#        print "par2 error:",par2_fitresult.getError()
	...
	fit_error_by_function = Get_Fit_Sigma( J_mass , nPseudo_data_fluc, modelVjet_test2 , frVjet_test2, constVjet_test2, par1_name , widthVjet_test2, par2_name  )
	print "fit_error_by_function:", fit_error_by_function
	...
	print "nFit_SR:",nFit_SR
        ...
        constVjet_test2.setVal(par1_plus )
        widthVjet_test2.setVal(par2_value )
        modelVjet_test2.plotOn(Jmass_frame3,RooFit.LineColor((RooFit.kGreen)+1),RooFit.Range("h_reasonable_range"))

        iFit_SR_error = modelVjet_test2.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("SRrange"))
        iFit_All_error = modelVjet_test2.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("h_reasonable_range"))
        nFit_SR_par1_plus = nPseudo_data_fluc * (  iFit_SR_error.getVal() / iFit_All_error.getVal() )

        iFit_SR_par1plus = iFit_SR_error.getVal()

        # par1 minus
        par1_minus = par1_value - par1_error
        constVjet_test2.setVal(par1_minus )
        widthVjet_test2.setVal(par2_value )
        modelVjet_test2.plotOn(Jmass_frame3,RooFit.LineColor((RooFit.kGreen)-1),RooFit.Range("h_reasonable_range"))

        iFit_SR_error = modelVjet_test2.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("SRrange"))
        iFit_All_error = modelVjet_test2.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("h_reasonable_range"))
        nFit_SR_par1_minus = nPseudo_data_fluc * (  iFit_SR_error.getVal() / iFit_All_error.getVal() )

        iFit_SR_par1minus = iFit_SR_error.getVal()

        # par2 plus
        par2_plus = par2_value + par2_error
        constVjet_test2.setVal(par1_value )
        widthVjet_test2.setVal(par2_plus  )
        modelVjet_test2.plotOn(Jmass_frame3,RooFit.LineColor((RooFit.kMagenta)+1),RooFit.Range("h_reasonable_range"))

        iFit_SR_error = modelVjet_test2.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("SRrange"))
        iFit_All_error = modelVjet_test2.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("h_reasonable_range"))
        nFit_SR_par2_plus = nPseudo_data_fluc * (  iFit_SR_error.getVal() / iFit_All_error.getVal() )

        iFit_SR_par2plus = iFit_SR_error.getVal()

        # par2 minus
        par2_minus = par2_value - par2_error
        constVjet_test2.setVal(par1_value )
        widthVjet_test2.setVal(par2_minus  )
        modelVjet_test2.plotOn(Jmass_frame3,RooFit.LineColor((RooFit.kMagenta)-1),RooFit.Range("h_reasonable_range"))

        iFit_SR_error = modelVjet_test2.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("SRrange"))
        iFit_All_error = modelVjet_test2.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("h_reasonable_range"))
        nFit_SR_par2_minus = nPseudo_data_fluc * (  iFit_SR_error.getVal() / iFit_All_error.getVal() )

        iFit_SR_par2minus = iFit_SR_error.getVal()

	# print nFit_SR of +/- 1 sigma uncertainty
	print ""
        print "nFit_SR:",nFit_SR
	print "nFit_SR_par1_plus:", nFit_SR_par1_plus
	print "nFit_SR_par1_minus:", nFit_SR_par1_minus
        print "nFit_SR_par2_plus:", nFit_SR_par2_plus
        print "nFit_SR_par2_minus:", nFit_SR_par2_minus

	# nFit_SR shift of +/- 1 sigma uncertainty  

	nFit_SR_shift_par1 = max( abs(nFit_SR_par1_plus - nFit_SR) , abs(nFit_SR - nFit_SR_par1_minus) )
        nFit_SR_shift_par2 = max( abs(nFit_SR_par2_plus - nFit_SR) , abs(nFit_SR - nFit_SR_par2_minus) )

#	print ""
	print "nFit_SR_shift_par1:", nFit_SR_shift_par1
        print "nFit_SR_shift_par2:", nFit_SR_shift_par2

	# row matrix * cor matrix * col matrix
	...
	iFit_SR_shift1 =  max( abs( iFit_SR_par1plus - iFit_SR_central ) , abs( iFit_SR_central - iFit_SR_par1minus ) ) 
        iFit_SR_shift2 =  max( abs( iFit_SR_par2plus - iFit_SR_central ) , abs( iFit_SR_central - iFit_SR_par2minus ) )

	rowM[0][0] = iFit_SR_shift1
        rowM[0][1] = iFit_SR_shift2

        colM[0][0] = iFit_SR_shift1
        colM[1][0] = iFit_SR_shift1

        resultM = rowM * ( cor * colM )
        iFit_SR_error_test_by_hand = math.sqrt( pow( resultM(0,0) , 2 ) )
	print ""
	print "iFit SR error test by hand"
	print "iFit_SR_central:", iFit_SR_central, " +/- ", iFit_SR_error_test_by_hand
	print ""
	...
	print "use RooFormulaVar"
        ...
        print "nFit_SR use RooRealVar: ", nFit_SR_formula.getVal()

        nFit_SR_formula_error = nFit_SR_formula.getPropagatedError( frVjet_test2 )
        print "nFit_SR_formula_error: ", nFit_SR_formula_error
	print ""
...
#                print "constVjet_case1: ", par1_value, " +/- ", par1_error
#		print "widthVjet_case1: ", par2_value, " +/- ", par2_error 		

		# get cor matrix
        	cor_case1 = frVjet_case1.correlationMatrix() ;


	        # par1 plus
        	par1_plus = par1_value + par1_error
        	constVjet_case1.setVal(par1_plus )
        	widthVjet_case1.setVal(par2_value )
                if times == times_to_plot : modelVjet_case1.plotOn(Jmass_frame4,RooFit.LineColor((RooFit.kGreen)+1),RooFit.Range("h_reasonable_range"))

        	iFit_SR_error = modelVjet_case1.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("SRrange"))
        	iFit_All_error = modelVjet_case1.createIntegral(jetMassArg,RooFit.NormSet(jetMassArg), RooFit.Range("h_reasonable_range"))
        	nFit_SR_par1_plus = nPseudo_data_fluc * (  iFit_SR_error.getVal() / iFit_All_error.getVal() )

                # par1 minus
                par1_minus = par1_value - par1_error
                constVjet_case1.setVal(par1_minus )
                widthVjet_case1.setVal(par2_value )
                if times == times_to_plot : modelVjet_case1.plotOn(Jmass_frame4,RooFit.LineColor((RooFit.kGreen)-1),RooFit.Range("h_reasonable_range"))
                ...
                widthVjet_case1.setVal(par2_plus )
                if times == times_to_plot : modelVjet_case1.plotOn(Jmass_frame4,RooFit.LineColor((RooFit.kMagenta)+1),RooFit.Range("h_reasonable_range"))
                ...
                par2_minus = par2_value - par2_error
                constVjet_case1.setVal(par1_value )
                widthVjet_case1.setVal(par2_minus )
                if times == times_to_plot : modelVjet_case1.plotOn(Jmass_frame4,RooFit.LineColor((RooFit.kMagenta)-1),RooFit.Range("h_reasonable_range"))
...
#        	print "nFit_SR_par2_minus:", nFit_SR_par2_minus
        	...
        	nFit_SR_shift_par1 = max( abs(nFit_SR_par1_plus - nFit_SR_case1) , abs(nFit_SR_case1 - nFit_SR_par1_minus) )
        	nFit_SR_shift_par2 = max( abs(nFit_SR_par2_plus - nFit_SR_case1) , abs(nFit_SR_case1 - nFit_SR_par2_minus) )

#		print "nFit_SR_shift_par1:", nFit_SR_shift_par1
#		print "nFit_SR_shift_par2:", nFit_SR_shift_par2

        	# row matrix * cor matrix * col matrix = Fit sigma
...
#        	colM.Print()
        	...
        	resultM = rowM * ( cor_case1 * colM )
        	sigma_Fit_case1 = math.sqrt( pow( resultM(0,0) , 2 ) )
#        	print "sigma_Fit_case1:", sigma_Fit_case1
#        	print ""
