...
//double Luminosity = 100000000;

// to draw all figures automatically
for(int v=1;v<=N_variables;v++){
//if(v!=3)continue;
...
TString figure_save_path = "/afs/cern.ch/user/y/yuchang/www/Optimize_4_cuts_with_Punzi_Significance_use_eleIsPassLoose/";
figure_path = figure_save_path + figure_path;
...
const int N_background_points = 4;
  ...
  string xsect_file_th = "./13TeV_xsec_DYJetHT.txt";
...
ifstream xsect_file2(xsect_file_th.c_str(), ios::in);
  ...
  while (xsect_file2.good()) {
    if(loop_counter> N_background_points)break;
...
double background_cross_section[N_background_points];
 ...
 double k_factor = 1.23;
  for(int i=0;i<v_DYJetHT_range.size();i++){
    ...
    background_cross_section[i] = v_xs_DYJetHT[i];
    cout<<"background_cross_section[i]: "<< background_cross_section[i] << endl;
...
double background_number_of_events[N_background_points];
...
//        double lowest_cut = 0; // for |dEta| and |d Rapidity| 
	...
	double first_bin_value = -999;
        double scan_width = 999;
             ...
             if (variable_flag==1){ lowest_cut=135 ; first_bin_value = 0;  scan_width = 5;}
        else if (variable_flag==2){ lowest_cut=20 ;  first_bin_value = 0;  scan_width = 5;}
  ...
  if(Eiko_efficiency_function){
		...
		// background eff
			...
			h_background_2nd_ele_pT_combine->Sumw2();
				...
				TH1D *h_background_EventWeight = (TH1D*) f_background -> Get("eventWeight");;
			        background_number_of_events[i] = h_background_EventWeight->Integral();
...
//                        double lumi = 1;
//                        h_background_2nd_ele_pT_combine_scale1->Sumw2();
...
//		        lumi = 1000;
//                          double lumi = Luminosity;
//			h_background_2nd_ele_pT_combine_scale2->Sumw2();
			h_background_2nd_ele_pT_combine_scale2->Scale( lumi );

			cout<<"bkg total: "<< h_background_2nd_ele_pT_combine_scale2->Integral()<<endl;
...
//                              cout<<"vy_sig_eff_erry[i]:  "<<vy_sig_eff_erry[i]   <<endl;
 ...
 //                             cout<<"vy_background_eff_erry[i]:  "<<vy_background_eff_erry[i]   <<endl;
  ...
  cout<< "highest_cut: " <<  highest_cut <<endl;
  double improve_rate = ( (vy_significance[hightest_bin] + vy_significance_erry[hightest_bin] )-(sig_noCut+sig_err_NoCut) ) /(sig_noCut+sig_err_NoCut);
  cout<<"compare defaut cut, highest cut improve: "<< improve_rate << endl; 
  cout<<endl;
...
if (variable_flag==1){
  ...
  if(k==2){xmin=150; xmax=250 ; ymin=0.22 ; ymax=0.28 ;}  // M1000
  ...
  if(k==7){xmin=300; xmax=500 ; ymin=0.5 ; ymax=0.6 ;}    // M2000
  if(k==8){xmin=300; xmax=500 ; ymin=0.75 ; ymax=0.875 ;} // M2500
  if(k==9){xmin=300; xmax=600 ; ymin=0.7 ; ymax=0.9 ;}    // M3000
...
if (variable_flag==2){
  ...
  if(k==2){xmin=30; xmax=150 ; ymin=0.2 ; ymax=0.3 ;}  // M1000
  ...
  if(k==7){xmin=50; xmax=210 ; ymin=0.4 ; ymax=0.6 ;}    // M2000
  if(k==8){xmin=50; xmax=150 ; ymin=0.7 ; ymax=0.8 ;} // M2500
  if(k==9){xmin=60; xmax=300 ; ymin=0.7 ; ymax=0.81 ;}    // M3000
...
if (variable_flag==3 || variable_flag==4){
  ...
  if(k==2){xmin=1.0; xmax=2.0 ; ymin=0.24 ; ymax=0.28 ;}  // M1000
  ...
  if(k==7){xmin=1.4; xmax=2.6 ; ymin=0.54 ; ymax=0.64 ;}    // M2000
  if(k==8){xmin=1.5; xmax=4.0 ; ymin=0.70 ; ymax=0.85 ;} // M2500
  if(k==9){xmin=1.75; xmax=3.75 ; ymin=0.74 ; ymax=0.85 ;}    // M3000
...

{
  ...
  // draw setting
gr_ratio->GetYaxis()->SetRangeUser(0.90, 1.3);
...
//	h_background_varies_mass->Sumw2();
